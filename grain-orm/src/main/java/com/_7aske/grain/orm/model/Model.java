...
public class Model {
	...
	public static <T extends Model> List<T> findAll(Class<T> clazz, Integer pageNumber, Integer pageSize) {
		Model instance = newInstance(clazz);
		return instance.doFindAll(clazz, pageNumber, pageSize);
	}
	...
	// @Temporary
	public <T extends Model> List<T> doFindAll(Class<T> clazz, Integer pageNumber, Integer pageSize) {
		return executeQuery(clazz, queryBuilder.select().join().page(pageNumber, pageSize).build());
	}
